...
@Component({
  selector: 'event-edit-theme',
  encapsulation: ViewEncapsulation.None,
  styles: [require('./edit.scss')],

  template: require('./edit.html')
})
export class EventEditTheme implements OnInit, AfterViewInit {
  ...
  loadData() {
   ...
   that._themeService.list(that.eventId).subscribe((json:any) => {
     that.totalItems = json.totalItems;
     that.items = json.themes;
   });
  }
  goto($event) {
    let that = this;
  ...
  onFormSubmit() {
    ...
    that._themeService.save(that.item).subscribe((json:any) => {
      if (json.code == 1) {
        that.hideModal();
        that.loadData();
      }
