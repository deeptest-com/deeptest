...
const expandNode = (keys: string[], e: any) => {
  ...
  setExpandedKeys('category-plan', currProject.value.id, expandedKeys.value)
}
...
const getExpandedKeysCall = debounce(async () => {
  getExpandedKeys('category-plan', currProject.value.id).then(async keys => {
    ...
    if (!expandedKeys.value || expandedKeys.value.length === 0) {
      ...
      await setExpandedKeys('category-plan', currProject.value.id, expandedKeys.value)
    }
  })
}, 300)
...
const selectStoredKeyCall = debounce(async () => {
  ...
  getSelectedKey('category-plan', currProject.value.id).then(async key => {
    console.log('key', key)
