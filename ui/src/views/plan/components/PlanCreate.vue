        ...
        <div class="plan-create-main">
            <a-form :label-col="labelCol" :wrapper-col="wrapperCol" :ref="formRef">
                ...
                <a-form-item
                    ...
                    >
                    <a-select v-model:value="modelRef.adminId" placeholder="请选择(默认当前用户)" :options="members" />
                </a-form-item>
                ...
                <a-form-item label="测试阶段">
                    <a-select v-model:value="modelRef.testStage" placeholder="请选择" :options="testStageArr" />
                </a-form-item>
                <a-form-item label="描述">
                    <a-textarea v-model:value="modelRef.desc" :auto-size="{ minRows: 2, maxRows: 5 }" />
                </a-form-item>
            </a-form>
        </div>
    </a-modal>
...
const store = useStore<{ Plan: PlanStateType, ProjectGlobal: ProjectStateType, User: UserStateType }>();
...
const modelRef = reactive({name: '', categoryId: null, adminId: (currentUser.value && currentUser.value.id) || null, testStage: null, desc: ''} as any)
const formRef = ref();
const testStageArr = [
    {
        label: '单元测试',
