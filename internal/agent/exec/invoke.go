...
func Invoke(req *domain.BaseRequest, key string, wsMsg *websocket.Message) (resp domain.DebugResponse, err error) {
	GetRequestProps(req)
	...
	if req.Method == consts.GET {
		resp, err = GetBy(*req, key, wsMsg)
	} else if req.Method == consts.POST {
		resp, err = PostBy(*req, key, wsMsg)
	} else if req.Method == consts.PUT {
		resp, err = PutBy(*req, key, wsMsg)
	} else if req.Method == consts.DELETE {
		resp, err = DeleteBy(*req, key, wsMsg)
	} else if req.Method == consts.PATCH {
		resp, err = PatchBy(*req, key, wsMsg)
	} else if req.Method == consts.HEAD {
		resp, err = HeadBy(*req, key, wsMsg)
	} else if req.Method == consts.CONNECT {
		resp, err = ConnectBy(*req, key, wsMsg)
	} else if req.Method == consts.OPTIONS {
		resp, err = OptionsBy(*req, key, wsMsg)
	} else if req.Method == consts.TRACE {
		resp, err = TraceBy(*req, key, wsMsg)
	}
