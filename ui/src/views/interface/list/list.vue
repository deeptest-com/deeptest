...
import {PaginationConfig, QueryParams, Interface} from '../data.d';
...
// import InterfaceCreate from "../edit/create.vue";
// import InterfaceEdit from "../edit/edit.vue";
...
const getList = debounce(async (current: number, categoryId: number): Promise<void> => {
  ...
  await store.dispatch('Interface/listInterface', {
    categoryId,
...
const remove = (id: number) => {
  ...
  Modal.confirm({
    ...
    cancelText: '取消',
    onOk: async () => {
      store.dispatch('Interface/removeInterface', id).then((res) => {
        console.log('res', res)
        if (res === true) {
          getList(1, nodeDataCategory.value.id)
