<template>
  <div class="request">
    <ConBoxTitle title="请求"></ConBoxTitle>
    <div class="content request-url">
      {{ data?.method || '' }} &nbsp; {{ data?.url || '' }}
    </div>

    <ConBoxTitle title="Params"></ConBoxTitle>
    <div class="request-headers">
      <a-table :dataSource="data?.queryParams || []" :columns="columns"
               :rowKey="(record,index) => {return index}"
               :rowClassName="(record,index) => {return record.name==='' ? 'hidden' : ''}"/>
    </div>

    <ConBoxTitle title="Headers"></ConBoxTitle>
    <div class="request-headers">
...
<!--  No Need
  <ConBoxTitle title="请求代码"></ConBoxTitle>
    <div class="content language-selector">
      <div :class="['language-item', selectIndex === index ? 'active' : '']" v-for="(item, index) in languageOptions"
           :key="index">
