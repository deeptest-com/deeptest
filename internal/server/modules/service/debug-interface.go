...
func (s *DebugInterfaceService) Load(loadReq domain.DebugReq) (debugData domain.DebugData, err error) {
	...
	debugData.BaseUrl, debugData.ShareVars, debugData.EnvVars, debugData.GlobalVars, debugData.GlobalParams =
		s.DebugSceneService.LoadScene(debugData.EndpointInterfaceId, debugData.ServeId, debugData.ServerId, debugData.ScenarioProcessorId, debugData.UsedBy)
...
func (s *DebugInterfaceService) Save(req domain.DebugData) (debugInterface model.DebugInterface, err error) {
	...
	if req.DebugInterfaceId <= 0 { // first time to save
		...
		s.ExtractorRepo.CloneFromEndpointInterfaceToDebugInterface(req.EndpointInterfaceId, debugInterface.ID, consts.InterfaceDebug)
		s.CheckpointRepo.CloneFromEndpointInterfaceToDebugInterface(req.EndpointInterfaceId, debugInterface.ID, consts.InterfaceDebug)
