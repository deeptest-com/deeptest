...
func GetVariable(session *ExecSession, processorId uint, nameOrExpr string) (ret domain.ExecVariable, err error) {
	/** for interface debug:
	...
	ret, err = getDynamicVariableFromScope(session, processorId, nameOrExpr)
	if ret.Name != "" {
		return
	}
	...
	ret, err = getVariableFromShareVar(session, nameOrExpr)
	if ret.Name != "" {
		return
	}

	// priority 3: Environment Vars, in project's serve settings
	ret, err = getVariableFromEnvVar(session, nameOrExpr)
	if ret.Name != "" {
		return
	}

	// priority 4: Global Vars, on project level
	ret, err = getVariableFromGlobalVar(session, nameOrExpr)
	if ret.Name != "" {
		return
	}
...
func SetVariable(session *ExecSession, processorId uint, variableName string, variableValue interface{},
	variableType consts.ExtractorResultType, scope consts.ExtractorScope) (
	newVariable domain.ExecVariable, err error) {

	scopeHierarchy := session.ScopeHierarchy
	scopedVariables := session.ScopedVariables
...
func ClearVariable(session *ExecSession, processorId uint, variableName string) (err error) {
	scopeHierarchy := session.ScopeHierarchy
	scopedVariables := session.ScopedVariables
