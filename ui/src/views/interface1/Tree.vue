...
import {parseSpecInLocalAgent, updateNodeName} from "./service";
import {Interface} from "./data";
...
const useForm = Form.useForm;
    ...
    const treeData = computed<any>(() => store.state.Interface1.treeData);
    const treeDataMap = computed<any>(() => store.state.Interface1.treeDataMap);
    ...
    const selectNode = (keys, e) => {
      ...
      store.dispatch('Interface1/getInterface', {id: selectedData.id, isLeaf: selectedData.isLeaf})
      if (selectedData.isLeaf) {
        store.dispatch('Interface1/getLastInvocationResp', selectedData.id)
        store.dispatch('Interface1/listInvocation', selectedData.id)
      }
    }
    ...
    const updateName = (id) => {
      ...
      updateNodeName(id, name).then((json) => {
        if (json.code === 0) {
          store.dispatch('Interface1/saveTreeMapItemProp', {id: id, prop: 'name', value: name})
          store.dispatch('Interface1/saveTreeMapItemProp', {id: id, prop: 'isEdit', value: false})
        }
      })
    }
    ...
    const renameNode = () => {
      ...
      Object.keys(treeDataMap.value).forEach((key) => {
        store.dispatch('Interface1/saveTreeMapItemProp', {id: key, prop: 'isEdit', value: false})
      })
      // treeDataMap.value[targetModelId].isEdit = true
      store.dispatch('Interface1/saveTreeMapItemProp', {id: targetModelId, prop: 'isEdit', value: true})
    ...
    const addNode = (mode, type) => {
      ...
      store.dispatch('Interface1/createInterface',
    ...
    const removeNode = () => {
      ...
      store.dispatch('Interface1/deleteInterface', targetModelId);
      selectNode([], null)
    }
    const clearMenu = () => {
    ...
    const onDrop = (info: DropEvent) => {
      ...
      store.dispatch('Interface1/moveInterface', {dragKey: dragKey, dropKey: dropKey, dropPos: dropPosition}).then(
          (result) => {
            if (result) {
    ...
    const importSubmit = async (data) => {
      ...
      if (parseResult.code === 0) {
        store.dispatch('Interface1/loadInterface').then((result) => {
          console.log(result)
