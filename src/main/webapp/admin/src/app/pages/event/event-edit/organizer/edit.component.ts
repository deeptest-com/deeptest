...
@Component({
  selector: 'event-edit-organizer',
  encapsulation: ViewEncapsulation.None,
  styles: [require('./edit.scss')],

  template: require('./edit.html')
})
export class EventEditOrganizer implements OnInit, AfterViewInit {
  ...
  onFormSubmit() {
    ...
    that._organizerService.save(that.item).subscribe((json:any) => {
      if (json.code == 1) {
        that.hideModal();
        that.loadData();
      }
  ...
  remove():void {
    ...
    that._organizerService.remove(that.item.id).subscribe((json:any) => {
      if (json.code == 1) {
        that.hideModal();
        that.loadData();
      }
